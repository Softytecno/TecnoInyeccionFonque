...
if (isset($_GET['placa'])) {
    ...
    $id_query = "SELECT * FROM tb_vehicles v JOIN tb_customers c ON v.vehi_id = c.cust_fk_vehicles WHERE v.vehi_license_plate = '$placa'";
    ...
    if ($id_result) {
        ...
        if ($id_row) {
            $id_placa = $id_row['cust_fk_vehicles'];

            $query = "SELECT * FROM tb_vehicles v JOIN tb_customers c ON v.vehi_id = c.cust_fk_vehicles WHERE v.vehi_id = $id_placa";
            ...
            $nombres = $row['cust_names'];
            $cedula = $row['cust_card'];
            $direccion = $row['cust_address'];
            $telefono = $row['cust_phone_number'];
            $ciudad = $row['cust_city'];
  ...
  if (isset($_POST['update'])) {
    ...
    $id_result = mysqli_query($conexion, $id_query);
    $id_row = mysqli_fetch_assoc($id_result);
    $fk_vehiculo = $id_row['cust_fk_vehicles'];

    $nombres = $_POST['cust_names'];
    $cedula = $_POST['cust_card'];
    $direccion = $_POST['cust_address'];
    $telefono = $_POST['cust_phone_number'];
    $ciudad = $_POST['cust_city'];
  
    $query = "UPDATE tb_customers set cust_names = '$nombres', cust_card = '$cedula', cust_address = '$direccion', cust_phone_number = '$telefono', cust_city = '$ciudad' WHERE cust_fk_vehicles = $fk_vehiculo";
        ...
        <div class="row">
            ...
            <div class="col-md-4"><br>
                <div class="card card-body" style="border-radius: 5px; box-shadow: 0 0 30px rgb(0, 0, 0, .568);">
                    <form action="editarCliente.php?placa=<?php echo $_GET['placa']; ?>" method="post">
                    <div class="alert alert-light col-md" role="alert" style="text-align: center;">
                            <a class="alert-link">
                                <h5 style="font-family:'Courier New'; font-size: 22px;">EDITAR CLIENTE</h5>
                            </a>
                        </div>
                    ...
                    <div class="form-group">
                        <input type="text" name="direccion" value="<?php echo $direccion ?>" class="form-control" placeholder="Direccion" autofocus="">
                    </div>
                    <div class="form-group">
                        <input type="text" name="telefono" value="<?php echo $telefono ?>" class="form-control" placeholder="Telefono" autofocus="">
                    </div>
                    <div class="form-group">
                        <input type="text" name="ciudad" value="<?php echo $ciudad ?>" class="form-control" placeholder="Ciudad" autofocus="">
                    </div>
